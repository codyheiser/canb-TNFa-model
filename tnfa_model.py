...
# Observable('TNFa_total', TNFa())
Observable('A20_total', A20())

# Expression('Test', Unbound_TNFa + Bound_TNFa)
Expression('r_activate', kb * Unbound_TNFa)
Expression('A20_modifier', ka20 / (ka20 + A20_total))
Expression('IKKK_activation', ka * A20_modifier)
Expression('A20_modifier2', (k2 + A20_total) / k2)
Expression('IKK_inactivation', k3 * A20_modifier2)
...
Rule('TNFR1_activates_IKKK',  TNFa(tnfr1=1) % TNFR1(tnfa=1) + IKKK(state='n') >> IKKK(state='a') +
     TNFa(tnfr1=1) % TNFR1(tnfa=1), IKKK_activation)
...
Rule('IKKa_deactivates', IKK(state='a') >> IKK(state='i'), IKK_inactivation)
...
for obs in model.observables:
    plt.plot(tspan, normalize(x[obs.name]), lw=2, label=obs.name)
    print(obs)
